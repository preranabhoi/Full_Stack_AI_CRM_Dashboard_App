...
export const generateLeadSummary = async (lead) => {
  const prompt = `You are an expert B2B sales analyst for a CRM called INFO CRM.
    Analyze the following sales lead and produce a concise assesment.
...
export const generateSalesInsights = async (pipelineStats) => {
  ...
  const schema = {
    ...
    required: ["headline", "insights", "recommendation", "healthScore"],
  };
